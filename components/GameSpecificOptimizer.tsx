...
export function GameSpecificOptimizer({
  game,
  packageId,
  onBack,
}: GameSpecificOptimizerProps) {
  ...
  const [optimizationScript, setOptimizationScript] =
    useState("");
  ...
  const loadSystemProfile = (
    type: "low-end" | "medium" | "high-end",
  ) => {
    setSystemType(type);
    ...
    tweaks.forEach((tweak) => {
      if (type === "low-end") {
        ...
        profile[tweak.key] =
          tweak.defaultValue !== undefined
            ? tweak.defaultValue
            : true;
      } else {
        // High-end: only enable non-graphics tweaks
        profile[tweak.key] =
          tweak.category !== "graphics"
            ? tweak.defaultValue !== undefined
              ? tweak.defaultValue
              : true
            : false;
    ...
    toast.success(
      `${type.toUpperCase()} profile loaded for ${gameNames[game]}`,
      {
  ...
  const applyOptimizations = () => {
    const script =
      SystemOptimizerService.generateGameSpecificScript(
        game,
        systemType,
        optimizations,
      );
    ...
    toast.success(
      `${gameNames[game]} optimization script downloaded!`,
      {
        description:
          "Run the .bat file as Administrator to apply optimizations",
      },
    );
  };
  ...
  const downloadRestoreScript = () => {
    const script =
      SystemOptimizerService.generateRestoreScript();
    const blob = new Blob([script], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
  ...
  const enableBoostMode = () => {
    ...
    toast.success(
      `🚀 BOOST MODE ACTIVATED FOR ${gameNames[game].toUpperCase()}!`,
      {
        description:
  ...
  const downloadNvidiaUniversalProfile = () => {
    const profile =
      NvidiaProfileInspectorService.generateUniversalProfile();
    ...
    toast.success("NVIDIA Universal Profile Downloaded!", {
      description:
        "Import this in NVIDIA Profile Inspector for all games",
    });
  };

  const downloadNvidiaGameProfile = () => {
    const profile =
      NvidiaProfileInspectorService.getProfileForGame(
        gameNames[game],
      );
    ...
    toast.success(
      `${gameNames[game]} NVIDIA Profile Downloaded!`,
      {
  ...
  const downloadAMDScript = () => {
    const script =
      AMDRadeonSettingsService.generateAMDOptimizationScript(
        gameNames[game],
      );
    const blob = new Blob([script], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    ...
    toast.success("AMD GPU Optimization Downloaded!", {
      description:
        "Run as administrator to optimize AMD GPU settings",
    });
  };
  ...
  const downloadAllGPUConfigs = () => {
    ...
    toast.success("All GPU configs downloading!", {
      description:
        "Universal, game-specific, and AMD configs ready",
    });
  };
  ...
  const performanceTweaks = getTweaksByCategory(
    game,
    "performance",
  );
  ...
  const tabs = [
    {
      ...
      icon: Rocket,
      count: quickTweaks.length,
    },
    {
      id: "engine",
      label: "Engine Specific",
      icon: Settings,
      count: engineTweaks.length,
    },
    {
      id: "graphics",
      label: "Graphics",
      icon: Gamepad2,
      count: graphicsTweaks.length,
    },
    {
      id: "performance",
      label: "Performance",
      icon: TrendingUp,
      count: performanceTweaks.length,
    },
    {
      id: "network",
      label: "Network",
      icon: Network,
      count: networkTweaks.length,
    },
    {
      id: "advanced",
      label: "Advanced",
      icon: Shield,
      count: advancedTweaks.length,
    },
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {tweaks.map((tweak) => (
          <div
            key={tweak.key}
            className="flex items-center justify-between p-4 bg-gray-900/50 backdrop-blur-xl rounded-lg hover:bg-gray-900/70 border border-gray-800 hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-500/10 transition-all"
          >
            ...
            <button
              ...
              className={`relative w-12 h-6 rounded-full transition-all flex-shrink-0 ml-4 ${
                optimizations[tweak.key]
                  ? `bg-${color}-500`
                  : "bg-gray-600"
              }`}
              style={{
                backgroundColor: optimizations[tweak.key]
                  ? color
                  : "#4b5563",
              }}
            >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-emerald-950 to-gray-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute bottom-0 left-0 w-96 h-96 bg-green-500/10 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "1.5s" }}
        ></div>
        <div
          className="absolute top-1/3 left-1/4 w-96 h-96 bg-teal-500/5 rounded-full blur-3xl animate-pulse"
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-white text-xl">
                    {gameNames[game]} Optimizer
                  </h1>
                  <p className="text-gray-400 text-sm">
                    Game-specific optimization tweaks
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-blue-400 mb-2">
                  100% Unique Tweaks for {gameNames[game]}
                </h4>
                <p className="text-gray-400 text-sm">
                  These optimizations are ONLY for{" "}
                  {gameNames[game]}. They modify{" "}
                  {gameNames[game]}'s config files, folders, and
                  settings. No generic system tweaks - every
                  tweak is specific to this game.
                </p>
                {game === "fivem" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → CitizenFX.ini, FiveM cache, GTA V settings
                  </p>
                )}
                {game === "minecraft" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → .minecraft folder, JVM args, options.txt
                  </p>
                )}
                {game === "cod" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → config_mp.cfg, Battle.net, Warzone cache
                  </p>
                )}
                {game === "fortnite" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → GameUserSettings.ini, Epic launcher
                  </p>
                )}
                {game === "valorant" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → Vanguard, Riot Client, VALORANT configs
                  </p>
                )}
                {game === "apex" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → autoexec.cfg, videoconfig.txt, Respawn
                  </p>
                )}
                {game === "csgo" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → CS autoexec, video.txt, Steam launch
                  </p>
                )}
                {game === "roblox" && (
                  <p className="text-cyan-400 text-xs mt-2">
                    → rbxfpsunlocker, ClientSettings, FastFlags
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="mb-6 p-6 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h4 className="text-green-400 mb-2">
                  🎮 GPU Optimization Configs - 1-Click Import
                </h4>
                <p className="text-gray-400 text-sm">
                  NO MORE MANUAL CLICKING! Download GPU configs
                  and import them instantly. Works for both
                  NVIDIA and AMD.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-black/30 rounded-lg border border-green-500/20">
                ...
                <p className="text-gray-400 text-xs mb-4">
                  2-second import instead of 10-minute manual
                  setup
                </p>
                ...
                <p className="text-gray-500 text-xs mt-3">
                  Download Profile Inspector → Import .nip →
                  Done!
                </p>
              </div>
              ...
              <div className="p-4 bg-black/30 rounded-lg border border-red-500/20">
                ...
                <p className="text-gray-400 text-xs mb-4">
                  Auto-apply all AMD GPU optimizations via
                  script
                </p>
                <div className="space-y-2">
                  <button
            ...
            <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
              <p className="text-yellow-400 text-xs">
                💡 <strong>Pro Tip:</strong> Import both
                Universal + Game-Specific NVIDIA profiles for
                best results. The universal sets the base,
                game-specific overrides for {gameNames[game]}.
              </p>
            </div>
          </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700">
            {activeTab === "quick" && (
              <>
                ...
                <p className="text-gray-400 text-sm mb-6">
                  Most impactful tweaks for {gameNames[game]}
                </p>
            ...
            {activeTab === "engine" && (
              <>
                ...
                <p className="text-gray-400 text-sm mb-6">
                  Optimizations for {gameNames[game]}'s game
                  engine
                </p>
            ...
            {activeTab === "graphics" && (
              <>
                ...
                <p className="text-gray-400 text-sm mb-6">
                  Visual quality tweaks for better FPS
                </p>
                ...
                <div className="mt-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        Advanced settings modify game
                        configuration files. Use "Restore
                        Defaults" if you experience issues.
                        Always backup your game settings before
                        applying.
                      </p>
                    </div>
                  </div>
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-blue-500/10 to-blue-900/10 backdrop-blur-xl border border-blue-500/20 rounded-xl p-4 hover:border-blue-500/40 hover:shadow-lg hover:shadow-blue-500/20 transition-all">
              ...
              <div className="text-white text-2xl">
                {getGameSpecificTweaks(game).length}
              </div>
            </div>
            <div className="bg-gradient-to-br from-green-500/10 to-green-900/10 backdrop-blur-xl border border-green-500/20 rounded-xl p-4 hover:border-green-500/40 hover:shadow-lg hover:shadow-green-500/20 transition-all">
              ...
              <div className="text-white text-2xl">
                {
                  Object.values(optimizations).filter(
                    (v) => v === true,
                  ).length
                }
              </div>
