import { Sparkles, AlertTriangle, Wifi, Pause } from 'lucide-react';
...
export function NetworkTweakCard({ 
  id, 
  title, 
  description, 
  warningLevel, 
  ...
  onToggle,
  userTier
}: NetworkTweakCardProps) {
  const isLocked = isPremium && userTier !== 'ELITE' && userTier !== 'FOUNDATION';

  // Determine icon based on title
  const getIcon = () => {
    if (title.includes('Disable') || title.includes('Stops')) {
